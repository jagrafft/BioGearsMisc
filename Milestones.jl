using CSV, DataFrames, JuliaDB, OnlineStats, Plots, StatsBase, StatsPlots, GR; gr(dpi=400)
...
# Statistics
## Descriptive
gi_desc = map(x -> NamedTuple{(:id,:len,:min,:max,:μ,:σ,:cov,:skewness,:kurtosis)}(tuple(x[2], round.([length(x[1]), minimum(x[1]), maximum(x[1]), mean(x[1]), std(x[1]), cov(x[1]), skewness(x[1]), kurtosis(x[1])], digits=2)...)), columns(select(gi, Between(8,38))) |> x -> zip(values(x), keys(x))) |> table

ob_desc = map(x -> NamedTuple{(:id,:len,:min,:max,:μ,:σ,:cov,:skewness,:kurtosis)}(tuple(x[2], round.([length(x[1]), minimum(x[1]), maximum(x[1]), mean(x[1]), std(x[1]), cov(x[1]), skewness(x[1]), kurtosis(x[1])], digits=2)...)), columns(select(ob, Between(8,39))) |> x -> zip(values(x), keys(x))) |> table

## Covarience
gi_cov = ((reduce((a,c) -> (push!(a[1], round.([v for v in map(x -> cov(select(gi, c), select(gi, x)), a[2])], digits=2)); a[2] = a[2][2:end]; a), colnames(gi)[8:38], init=[[], colnames(gi)[8:38]])[1] .|> x -> vcat([Missing for z in 1:1:(31-length(x))], x)) |> m -> zip(colnames(gi)[8:38], m) .|> x -> NamedTuple{tuple(:features, colnames(gi)[8:38]...)}(tuple([x[1], x[2]...]...))) |> table

ob_cov = ((reduce((a,c) -> (push!(a[1], round.([v for v in map(x -> cov(select(ob, c), select(ob, x)), a[2])], digits=2)); a[2] = a[2][2:end]; a), colnames(ob)[8:39], init=[[], colnames(ob)[8:39]])[1] .|> x -> vcat([Missing for z in 1:1:(32-length(x))], x)) |> m -> zip(colnames(ob)[8:39], m) .|> x -> NamedTuple{tuple(:features, colnames(gi)[8:39]...)}(tuple([x[1], x[2]...]...))) |> table

feat_cov = [x for x in reduce((a,c) -> (push!(a[1], (i=a[2], N=sample_size, feature=c, covariance=([gi, ob] .|> x -> select(x, :introduce_self) |> x -> StatsBase.sample(x, sample_size, replace=false)) |> x -> round(cov(x[1], x[2]), digits=2))); a[2] = a[2] + 1; a), intersect(colnames(gi), colnames(ob))[8:34], init=[[], 1])[1]] |> table

## 
[("DescriptiveStats_OBD.csv", ob_desc), ("DescriptiveStats_GIBleed.csv", gi_desc), ("Covariance_GIBleed.csv", gi_cov), ("Covariance_OBD.csv", ob_cov), ("Covariance_by_common_feature.csv", feat_cov)] .|> x -> CSV.write(x...)
