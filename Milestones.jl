...
feat_ost = [x for x in reduce((a,c) -> (push!(a[1], (i=a[2], feature=c, pval=(([gi, ob] .|> x -> select(x, c)) |> x -> round(UnequalVarianceTTest(x[1], x[2]) |> pvalue, digits=2)))); a[2] = a[2] + 1; a), intersect(colnames(gi), colnames(ob))[8:34], init=[[], 1])[1]] |> table
...
## Write CSV
[("DescriptiveStats_GIBleed.csv", gi_desc), ("DescriptiveStats_OBD.csv", ob_desc), ("UneqVarTTest_GIBleed.csv", gi_ost), ("UneqVarTTest_OBD.csv", ob_ost), ("UneqVarTTest_by_common_feature.csv", feat_ost), ("Covariance_GIBleed.csv", gi_cov), ("Covariance_OBD.csv", ob_cov), ("Covariance_by_common_feature.csv", feat_cov)] .|> x -> CSV.write(x...)
