...
pairs = Dict(); ["eti:start", "eti:stop", "laryngoscopy:start", "laryngoscopy:stop"] .|> ev -> (_df = DataFrame(); tuplesbykey(df, :ev_code, [:i, :t], [ev]) |> y -> (keys(y[1].vals) .|> z -> _df[z] = (y .|> v -> v.vals[z])); pairs[ev] = _df; pairs);

drugs = tuplesbykey(df, :ev_code, [:i, :t, :result, :notes], ["Fentanyl", "Propofol", "Rocuronium", "Vecuronium", "Succinylcholine"]);
